...
int main(int argc, char** argv)
{
	...
	ifstream processi("expprocess.txt");
	...
	ifstream arrivali("poissonarrival.txt");
	...
	{
		arrival.open("poissonarrival.txt");
		process.open("expprocess.txt");
		...
		for(int h = 0; h < w.size(); h++){
			variance = variance + pow((w[h] - average_wait),2);
			}	
		...
		cout<<"\n\nNumber of Customer does not match Process times--CHECK AGAIN";
